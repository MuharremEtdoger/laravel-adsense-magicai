...
{!! $google_adsenses->horizontal !!} <!-- Reklam Kodunun Çağrıldığı Bölüm 1 -->
	...
	<div class="container">
		<div class="p-10 border rounded-[50px] max-sm:px-6 max-sm:py-16">
			...
			<div class="max-lg:grid-cols-2 max-md:grid-cols-1 grid grid-cols-3 gap-3">
				...
				@endphp					
				    @if($sayac==6)
					{!! $google_adsenses->square_ads !!} <!-- Reklam Kodunun Çağrıldığı Bölüm 2 -->
					@else
					<x-box style="3" title="{!! __($tool->title) !!}{!! $sayac !!}" desc="{!! __($tool->description) !!}">
						<x-slot name="image">
